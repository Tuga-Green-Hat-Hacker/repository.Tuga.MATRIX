...
# - script.module.AceStreamPlayer
# - @AceStreamMOD (https://t.me/AceStreamMOD)
# - 
#
# This file is part of AceStream for Kodi
#
# AceStream for Kodi is free software: you can redistribute it and/or modify
...
class OSD(object):
    def __init__(self, stats):
        ...
        # Background
        self.AceStreamPlayer_background = xbmcgui.ControlImage(x=posX, y=posY, width=window_w, height=window_h,
                                                     filename=os.path.join(runtime_path, 'resources', 'media' , 'background.png'))
        # icon
        self.AceStreamPlayer_icon = xbmcgui.ControlImage(x=posX + 25, y=posY + 15, width=38, height=28,
                                               filename=os.path.join(runtime_path, 'resources', 'media' , 'acestreamlogo.png'))
        # title
        self.AceStreamPlayer_title = xbmcgui.ControlLabel(x=posX + 78, y=posY + 13, width=window_w - 10, height=30,
                                                label="AceStreamPlayer", font=font_max, textColor='0xFFEB9E17')
        # sep
        self.AceStreamPlayer_sep1 = xbmcgui.ControlImage(x=posX + 5, y=posY + 55, width=window_w - 10, height=1,
                                               filename=os.path.join(runtime_path, 'resources', 'media', 'separator.png'))
        # Stats
        self.AceStreamPlayer_status = xbmcgui.ControlLabel(x=posX + 25, y=posY + 80, width=window_w - 10, height=30, font=font_min, label=translate(30009) % '')
        self.AceStreamPlayer_speed_down = xbmcgui.ControlLabel(x=posX + 25, y=posY + 100, width=window_w - 10, height=30, font=font_min, label=translate(30009) % 0)
        self.AceStreamPlayer_speed_up = xbmcgui.ControlLabel(x=posX + 25, y=posY + 120, width=window_w - 10, height=30, font=font_min, label=translate(30011) % 0)
        self.AceStreamPlayer_peers = xbmcgui.ControlLabel(x=posX + 25, y=posY + 140, width=window_w - 10, height=30, font=font_min, label=translate(30012) % 0)
        self.AceStreamPlayer_downloaded = xbmcgui.ControlLabel(x=posX + 25, y=posY + 170, width=window_w - 10, height=30, font=font_min, label=translate(30013) % 0)
        self.AceStreamPlayer_uploaded = xbmcgui.ControlLabel(x=posX + 25, y=posY + 190, width=window_w - 10, height=30, font=font_min, label=translate(30014) % 0)

        # sep
        self.AceStreamPlayer_sep2 = xbmcgui.ControlImage(x=posX + 5, y=posY + window_h - 30, width=window_w - 10, height=1,
                                               filename=os.path.join(runtime_path, 'resources', 'media', 'separator.png'))

    def update(self,**kwargs):
        if self.showing:
            status = {'dl':translate(30007), 'prebuf': translate(30008) %(self.stats.progress) + '%'}
            self.AceStreamPlayer_status.setLabel(translate(30009) % status.get(self.stats.status, self.stats.status))
            self.AceStreamPlayer_speed_down.setLabel(translate(30010) % self.stats.speed_down)
            self.AceStreamPlayer_speed_up.setLabel(translate(30011) % self.stats.speed_up)
            self.AceStreamPlayer_peers.setLabel(translate(30012) % self.stats.peers)
            self.AceStreamPlayer_downloaded.setLabel(translate(30013) % (self.stats.downloaded // 1048576))
            self.AceStreamPlayer_uploaded.setLabel(translate(30014) % (self.stats.uploaded // 1048576))

    def show(self):
        self.window.addControl(self.AceStreamPlayer_background)
        self.window.addControl(self.AceStreamPlayer_icon)
        self.window.addControl(self.AceStreamPlayer_title)
        self.window.addControl(self.AceStreamPlayer_sep1)
        self.window.addControl(self.AceStreamPlayer_status)
        self.window.addControl(self.AceStreamPlayer_speed_down)
        self.window.addControl(self.AceStreamPlayer_speed_up)
        self.window.addControl(self.AceStreamPlayer_peers)
        self.window.addControl(self.AceStreamPlayer_downloaded)
        self.window.addControl(self.AceStreamPlayer_uploaded)
        self.window.addControl(self.AceStreamPlayer_sep2)
        self.showing = True

    def hide(self):
        self.showing = False
        self.window.removeControl(self.AceStreamPlayer_background)
        self.window.removeControl(self.AceStreamPlayer_icon)
        self.window.removeControl(self.AceStreamPlayer_title)
        self.window.removeControl(self.AceStreamPlayer_sep1)
        self.window.removeControl(self.AceStreamPlayer_status)
        self.window.removeControl(self.AceStreamPlayer_speed_down)
        self.window.removeControl(self.AceStreamPlayer_speed_up)
        self.window.removeControl(self.AceStreamPlayer_peers)
        self.window.removeControl(self.AceStreamPlayer_downloaded)
        self.window.removeControl(self.AceStreamPlayer_uploaded)
        self.window.removeControl(self.AceStreamPlayer_sep2)
...
class MyPlayer(xbmc.Player):
    ...
    def playStream(self, stream, title='', iconimage='', plot='', init_time=0.0):
        ...
        art = {'icon': iconimage if iconimage else os.path.join(runtime_path, 'resources', 'media', 'icon_aces_AceStreamPlayer.png')}
        listitem.setArt(art)
        ...
        if self.AVStarted:
            if self.current_time > 180:
                add_historic({'infohash': stream.infohash,
                                'title': title,
    ...
    def onPlayBackEnded(self):
        logger("PLAYBACK ENDED") # Network outage or end of video
        self.is_active = False

    def onPlayBackStopped(self):
        logger("PLAYBACK STOPPED") # Stopped by user or not started by http 429
        self.is_active = False
...
def get_historic():
    historic = list()
    settings_path= os.path.join(data_path, "historic.json")
    if os.path.isfile(settings_path):
        try:
            historic = load_json_file(settings_path)
        except Exception:
...
def add_historic(contenido):
    ...
    if not trobat:
        historic.insert(0, contenido)
        dump_json_file(historic[:10], settings_path)
...
def acestreams(id=None, url=None, infohash=None, title="", iconimage="", plot=""):
    ...
    #url = 'https://comandostorrent.net/category/top-cine/'
    #infohash = ''
    ...
    if not server.available:
        ...
        if system_platform == "windows":
            ...
            elif architecture == 'arm' and not root:
                try:
                    data = ''
        ...
        if acestream_executable and not server.available:
            ...
            logger("unknown platform: %s" % system_platform)
    ...
    try:
        ...
        if not acestream_executable or system_platform == 'android':
            while not d.iscanceled() and not server.available and time.time() < timedown and error_flag == False:
                ...
                progress = int((seg * 100) / get_setting("time_limit"))
                line1 = translate(30033)
                line2 = translate(30006) % seg
                try:
                    d.update(progress, line1, line2)
                except:
                    d.update(progress, '\n'.join([line1, line2]))
                time.sleep(1)
            ...
            if not server.available:
                ...
                raise Exception("action canceled or timeout")
            ...
            if d.iscanceled() or time.time() >= timedown or error_flag == True: # Time ended or canceled
                if engine.running:
                    engine.stop()
                d.close()
        ...
        while not d.iscanceled() and time.time() < timedown and (not stream.status or stream.status != 'dl') and error_flag == False:
            ...
            if not stream.status:
                ...
                line1 = translate(30008) %(progress) + '%'
            ...
            try:
                d.update(progress, line1, line2, line3)
            except:
...
def notification_info(*args,**kwargs):
    ...
    #xbmcgui.Dialog().notification('Acestream %s' % transmitter, msg, os.path.join(runtime_path, 'resources', 'media', 'icon_aces_AceStreamPlayer.png'))
...
def kill_process():
    ...
    if not server.available:
        logger("Enclosed Acestream Engine")
        xbmcgui.Dialog().notification(HEADING, translate(30035),
                                      os.path.join(runtime_path, 'resources', 'media', 'icon_aces_AceStreamPlayer.png'))
        return True
    else:
        logger("Acestream motor NOT closed")
...
def run(item):
    ...
    if item.action == "mainmenu":
    ...
    elif item.action == "historic":
        for it in get_historic():
        ...
        if item.id:
            ...
            last_id = get_setting("last_id", "56bb17b376ec52b5e58679d87ed2bd9e70be2175")
    ...
    if sys.argv[2]:
        try:
            ...
            for c in sys.argv[2][1:].split('&'):
                k, v = c.split('=')
                arguments[k] = urllib_parse.unquote_plus(six.ensure_str(v))

            logger("External call: %s" %arguments)
            action = arguments.get('action', '').lower()

            if action == 'play' and (arguments.get('id') or arguments.get('url') or arguments.get('infohash')):
                if arguments.get('url') and arguments.get('url').lower().endswith('.torrent'):
                    infohash = read_torrent(arguments.get('url'), arguments.get('headers'))
                    if infohash:
                        arguments['infohash'] = infohash
                        arguments['url'] = None
                    else:
                        exit(0)
                elif arguments.get('url') and arguments.get('url').lower().startswith('magnet:'):
                    infohash = re.findall('xt=urn:btih:([a-f0-9]+)', arguments.get('url'), re.I)
                    if infohash:
                        arguments['infohash'] = infohash[0]
                        arguments['url'] = None
                    else:
                        exit(0)

                acestreams(id=arguments.get('id'),
                           url=arguments.get('url'),
                           infohash=arguments.get('infohash'),
                           title=arguments.get('title'),
                           iconimage=arguments.get('iconimage'),
